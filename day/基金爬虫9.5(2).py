...
con = pymysql.connect(host='192.168.10.168',user='root',password='123456',database='all_info_data')
cursor = con.cursor()
missing=list()
already = list()
...
items=pd.read_excel('info_20221026.xlsx',engine='openpyxl',dtype={'id':'str'})  #不支持xlsx文件，加engine='openpyxl' #info_result是上上个季度的
...
con = pymysql.connect(host='192.168.10.168', user='root', password='123456', database='project')
...
data_final_today.to_sql('{}'.format(date_work),con='mysql+pymysql://root:123456@192.168.10.168/rank_data?charset=utf8',if_exists='replace')
