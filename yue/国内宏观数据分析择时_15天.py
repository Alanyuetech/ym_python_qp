# -*- coding: utf-8 -*-
"""
Created on Tue Mar 14 10:16:44 2023
...
def total_function(index_bs,fjd_n,bdl_bs,index_bdl_bs,timing_bs):
    ...
    # hs300_ym = period_chg(hs300,timing=timing_bs)[['年月','chg_qual']].rename(columns={'chg_qual':'hs300_chg_qual'})
    hs300_old_ym = period_chg(hs300,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'hs300_old_chg_qual'})

    "指数未来15天涨跌幅"
    hs300_ym = hs300[['年月']].drop_duplicates().reset_index(drop=True)
    hs300_columns = list(hs300.columns) 
   
    if timing_bs=='15':
        for index,row in hs300_ym[:].iterrows():
            try:         
                hs300_ym.loc[index,'chg'] = hs300[(hs300['年月']=='{}'.format(row['年月']))&(hs300['日']>='{}'.format(timing_bs))].head(1).reset_index(drop=True).loc[0,'{}'.format(hs300_columns[1])] - hs300[(hs300['年月']=='{}'.format(row['年月']))&(hs300['日']>='{}'.format(timing_bs))].tail(1).reset_index(drop=True).loc[0,'{}'.format(hs300_columns[1])]
                hs300_ym['chg_qual'] = hs300_ym['chg'].map(lambda x:1 if x>0 else(-1 if x<0 else 0))
                hs300_ym['timing_bs'] = timing_bs
            except:
                pass
    elif timing_bs=='32':
        for index,row in hs300_ym[:].iterrows():
            try:         
                hs300_ym.loc[index,'chg'] = hs300[(hs300['年月']=='{}'.format(hs300_ym.loc[index-1,'年月']))&(hs300['日']<='14')].head(1).reset_index(drop=True).loc[0,'{}'.format(hs300_columns[1])] - hs300[(hs300['年月']=='{}'.format(hs300_ym.loc[index-1,'年月']))&(hs300['日']<='14')].tail(1).reset_index(drop=True).loc[0,'{}'.format(hs300_columns[1])]
                hs300_ym['chg_qual'] = hs300_ym['chg'].map(lambda x:1 if x>0 else(-1 if x<0 else 0))
                hs300_ym['timing_bs'] = timing_bs
            except:
                pass
        ...
        pass
    hs300_ym = hs300_ym[['年月','chg_qual']].copy().rename(columns={'chg_qual':'hs300_chg_qual'})
    
    ####################################################
    "美元中间价"
    usdcny = time_bs_creative(usdcny,bs=['年月','日'])   #增加年，月，日，季度时间标识
    usdcny_ym = period_chg(usdcny,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'usdcny_chg_qual'})
    ####################################################
    "ppi当月同比"
    ppi_tb = time_bs_creative(ppi_tb,bs=['年月','日'])   #增加年，月，日，季度时间标识
    ppi_tb['预期误差'] = ppi_tb['今值(%)']-ppi_tb['预测值(%)']
    ppi_ym1 = period_chg(ppi_tb[['date','今值(%)','年月','日']].copy(),timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'ppi_tb_qual'})
    ppi_ym2 = period_chg(ppi_tb[['date','预期误差','年月','日']].copy(),timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'ppi_error_qual'})
    ...
    cpi_ym1 = period_chg(cpi_tb[['date','今值(%)','年月','日']].copy(),timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'cpi_tb_qual'})
    cpi_ym2 = period_chg(cpi_tb[['date','预期误差','年月','日']].copy(),timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'cpi_error_qual'})
    ...
    wind_alla_ym = period_chg(wind_alla,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'wind_alla_chg_qual'})
    ####################################################
    "wind全A 120日滚动波动率" 
    wind_alla_bdl = time_bs_creative(wind_alla_bdl,bs=['年月','日'])   #增加年，月，日，季度时间标识
    wind_alla_bdl_ym = period_chg(wind_alla_bdl,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'wind_alla_bdl_chg_qual'})
    
    ####################################################
    "A股指数  120日滚动波动率" 
    hs300_bdl = time_bs_creative(hs300_bdl,bs=['年月','日'])   #增加年，月，日，季度时间标识
    hs300_bdl_ym = period_chg(hs300_bdl,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'hs300_bdl_chg_qual'})
    ####################################################    
    "官方制造业PMI"
    ...
    for index,row in pmi.iterrows():
        if row['日']>=timing_bs:
            ...
            pass
    pmi_ym = period_chg(pmi,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'pmi_chg_qual'})
    ...
    for index,row in cx_pmi.iterrows():
        if row['日']>=timing_bs:
            ...
            pass
    cx_pmi_ym = period_chg(cx_pmi,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'cx_pmi_chg_qual'})
    ...
    for index,row in indtr_va.iterrows():
        if row['日']>=timing_bs:
            ...
            pass
    indtr_va_ym = period_chg(indtr_va,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'indtr_va_chg_qual'})
    ...
    cfets_index_ym = period_chg(cfets_index,timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'cfets_chg_qual'})
    ####################################################
    "中债企业债到期收益率(AA):1个月" 
    "中债国开债到期收益率:10年" 
    cnbond = time_bs_creative(cnbond,bs=['年月','日']) 
    cbcb_ym = period_chg(cnbond[['date','cbcb','年月','日']].copy(),timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'cbcb_chg_qual'})
    csid_ym = period_chg(cnbond[['date','csid','年月','日']].copy(),timing=timing_bs)[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'csid_chg_qual'})
    ...
    data_all_adjust['timing_bs'] = timing_bs
    
    
    return data_all_adjust
    
    
def statistics_data(data_all_adjust,fjd_n): 
    "1： 去掉23年2月   1："
    data_stat_b = data_all_adjust[(data_all_adjust['年月']<='202303')&(data_all_adjust['年月']>='201601')].copy()
...
data_stat = pd.DataFrame()
for index_bdl_bs in ['涨跌幅','收盘价']:
    for bdl_bs in ['收盘价']:
        for fjd_i in range(-3,7):
            for index,row in index_bs_df.iterrows():
                data_all_adjust_1 = total_function(row['预测指数代码'],fjd_i,bdl_bs,index_bdl_bs,'15')
                data_all_adjust_2 = total_function(row['预测指数代码'],fjd_i,bdl_bs,index_bdl_bs,'32')
                data_all_adjust = pd.concat([data_all_adjust_1,data_all_adjust_2]).sort_values(['年月','timing_bs'],ascending=[0,0])

                
                data_stat_bb = statistics_data(data_all_adjust,fjd_i)
