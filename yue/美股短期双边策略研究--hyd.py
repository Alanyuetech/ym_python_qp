# -*- coding: utf-8 -*-
"""
Created on Mon Dec 12 13:04:12 2022
...
def single_stock_operation(single_stock_id,start_date,s_period,l_period,holding_days):
    ...
    stock_us_data_b = ak.stock_us_hist(symbol=single_stock_id, period="daily", 
...
def short_strategy(symbol_i='自定义名单',start_date='20220101',parameters=[5,10,5]):
    ...
    if symbol_i!='自定义名单':
        ...
        stock_us_list['stock_id'] = stock_us_list['代码'].str.rsplit('.',expand = True)[1]     
        stock_us_famous_spot = pd.read_excel('股票池.xlsx',engine='openpyxl')[-30:][['代码','总市值']].rename(columns={'代码':'stock_id'}).reset_index(drop=True)
        stock_us_famous_spot = pd.merge(stock_us_famous_spot,stock_us_list[['代码','stock_id']],how='left',on='stock_id')[['代码','stock_id','总市值']]
...
for symbol_i in symbol_list:
    for start_date in start_date_list:
        for parameters in [[3,5,3],[3,5,5],[3,5,10],[3,10,3],[3,10,5],[3,10,10],[5,10,3],[5,10,5],[5,10,10]]:
            ...
            # trading_list_perform_ana_b,trading_list = short_strategy('自定义名单',start_date,parameters)   #自定义股票名单
