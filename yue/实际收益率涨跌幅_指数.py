# -*- coding: utf-8 -*-
"""
Created on Thu Aug  5 15:52:56 2021
...
def get_dfii10_index(ans_index):
    ...
    "计算单日实际收益率涨跌 的  标准差"
    std_1d = ans_inf['单日实际收益率涨跌'].std()
    mean_1d = ans_inf['单日实际收益率涨跌'].mean()
    
    "1~3个标准差之外的的数据"
    ans_std_1 = ans_inf[(ans_inf['单日实际收益率涨跌']>=mean_1d+1*std_1d)|(ans_inf['单日实际收益率涨跌']<=mean_1d-1*std_1d)]
    ans_std_2 = ans_inf[(ans_inf['单日实际收益率涨跌']>=mean_1d+2*std_1d)|(ans_inf['单日实际收益率涨跌']<=mean_1d-2*std_1d)] 
    ans_std_3 = ans_inf[(ans_inf['单日实际收益率涨跌']>=mean_1d+3*std_1d)|(ans_inf['单日实际收益率涨跌']<=mean_1d-3*std_1d)]
    ...
    "单日实际收益率涨跌  处于1，2，3个标准差之外  对应的指数未来1~30天的涨幅"
    ansstd1_ans130 = pd.merge(left=ans_1_30,right=ans_std_1[['日期','单日实际收益率涨跌']],how='left',on='日期')
    ansstd1_ans130 = ansstd1_ans130.dropna(subset=['单日实际收益率涨跌'])
    
    ansstd2_ans130 = pd.merge(left=ans_1_30,right=ans_std_2[['日期','单日实际收益率涨跌']],how='left',on='日期')
    ansstd2_ans130 = ansstd2_ans130.dropna(subset=['单日实际收益率涨跌'])
    
    ansstd3_ans130 = pd.merge(left=ans_1_30,right=ans_std_3[['日期','单日实际收益率涨跌']],how='left',on='日期')
    ansstd3_ans130 = ansstd3_ans130.dropna(subset=['单日实际收益率涨跌'])
...
"把def里的一些变量重新运行一下"
data_list = pd.Series()
data_list['实际收益率涨幅平均值'] = mean_1d
data_list['实际收益率涨幅标准差'] = std_1d
...
data_list['小于1个标准差数据量'] = len(ans_std_1.drop(ans_std_1[ans_std_1['单日实际收益率涨跌']<mean_1d-1*std_1d].index))
data_list['大于1个标准差数据量'] = len(ans_std_1.drop(ans_std_1[ans_std_1['单日实际收益率涨跌']>mean_1d+1*std_1d].index))
data_list['小于2个标准差数据量'] = len(ans_std_2.drop(ans_std_2[ans_std_2['单日实际收益率涨跌']<mean_1d-2*std_1d].index))
data_list['大于2个标准差数据量'] = len(ans_std_2.drop(ans_std_2[ans_std_2['单日实际收益率涨跌']>mean_1d+2*std_1d].index))
data_list['小于3个标准差数据量'] = len(ans_std_3.drop(ans_std_3[ans_std_3['单日实际收益率涨跌']<mean_1d-3*std_1d].index))
data_list['大于3个标准差数据量'] = len(ans_std_3.drop(ans_std_3[ans_std_3['单日实际收益率涨跌']>mean_1d+3*std_1d].index))
