# -*- coding: utf-8 -*-
"""
Created on Fri Oct 21 13:09:04 2022
...
####################################################
def period_chg(df,timing='15',bs='hardline'):
    "timing 每个月的 timing日 作为计算的起始周期"
    df_ym = df[['年月']].drop_duplicates().reset_index(drop=True)
    df_columns = list(df.columns)  
    if bs=='hardline':
        for index,row in df_ym[:].iterrows():
            try:
                df_ym.loc[index,'month_start'] = df[(df['年月']==row['年月'])&(df['日']>=timing)].tail(1).reset_index(drop=True).loc[0,'{}'.format(df_columns[1])]
                df_ym.loc[index,'month_final'] = df[(df['年月']==row['年月'])&(df['日']<timing)].head(1).reset_index(drop=True).loc[0,'{}'.format(df_columns[1])]
            except:
                pass
    elif bs=='backfill':
        for index,row in df_ym[:].iterrows():
            try:
                df_ym.loc[index,'month_start'] = df[(df['年月']==row['年月'])&(df['日']<=timing)].head(1).reset_index(drop=True).loc[0,'{}'.format(df_columns[1])]
                df_ym.loc[index,'month_final'] = df[(df['年月']==row['年月'])&(df['日']<timing)].head(1).reset_index(drop=True).loc[0,'{}'.format(df_columns[1])]
            except:
                pass
        df_ym = df_ym.dropna(axis=0,how='any')
    else:
        pass
    df_ym['month_final_shift'] = df_ym['month_final'].shift(1)
    df_ym['month_start_shift'] = df_ym['month_start'].shift(-1)
    df_ym['chg'] = df_ym['month_final_shift'] - df_ym['month_start'] 
    df_ym['chg_old'] = df_ym['month_final'] - df_ym['month_start_shift']
...
def total_function(index_bs,fjd_n,bdl_bs,index_bdl_bs):
    ...
    cfets_index_ym = period_chg(cfets_index,bs='backfill')[['年月','chg_old_qual']].rename(columns={'chg_old_qual':'cfets_chg_qual'})
    ####################################################
    "中债企业债到期收益率(AA):1个月" 
    "中债国开债到期收益率:10年" 
    ...
    dfs = [hs300_ym,pmi_ym,cx_pmi_ym,ppi_tb_ym[['年月','ppi_error_qual']],cpi_tb_ym[['年月','cpi_error_qual']]
           ,cfets_index_ym,usdcny_ym,cbcb_ym,csid_ym]   # 这里未来是可以输入参数来循环的
...
data_stat = pd.DataFrame()
for index_bdl_bs in ['收盘价','涨跌幅']:  #正常状态下只有一个，需要讨论不同情况的时候加上     ,'涨跌幅'
    for bdl_bs in ['收盘价']:
