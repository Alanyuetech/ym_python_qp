# -*- coding: utf-8 -*-
"""
Created on Mon Sep  7 14:49:53 2020
...
# data_final.drop(['单位净值', '累计净值', '日增长率', '近1周', '近1月', '近3月', '近6月', '近1年', '近2年', '近3年',
#        '今年来', '成立来', 'days', 'name', 'kind', 'size'],axis=1,inplace=True)
...
for i in range(2,27):
    ...
    data_middel=pd.merge(data_middel,data[['week'+str(i),'id']],how='left',on='id')    # 之前是用inner   出现data_middel 和 data1的size不同，尝试改成  left
    now=After
...
#     data_middel[x] = data_middel[x].str.strip('%').astype(float)/100     #  不需要把百分号去掉


data_middel=data_middel.fillna(0)

"下方index可能会不一样 ：从头开始重新跑，保证 data_middel 和 data1的size相同 修改日期 20220210"
data_middel.index=data1.index
...
# items=read_items('规模.csv')
items=pd.read_excel('info_20230128.xlsx',engine='openpyxl',dtype={'id':'str'})   #规模.csv   size数据不准确
...
# data_final.to_excel('两周时间段爬取{}.xlsx'.format(date_work),index=False)
...
def data_process_first(data_final):        
    ...
    data_final['size']=data_final['size'].replace('--',np.nan)
